...
class _SignListScreenState extends State<SignListScreen> {
  ...
  Future<void> fetchSigns() async {
    ...
    try {
      final response = await http.get(Uri.parse('${Config.baseUrl}/panneauxRoutes/exemple/arret_stop'));
      ...
      if (response.statusCode == 404) {
        ...
        // Save data to Hive
        await box.put('signs_arret_stop', signsList);
...
// Function to load signs from Hive if offline or fetching fails
  void loadFromHive(Box box) {
    final cachedData = box.get('signs_arret_stop', defaultValue: []);
