...
class Priority_sc extends StatefulWidget {
  @override
  _Priority_scState createState() => _Priority_scState();
}

// ✅ Sign List Screen
class _Priority_scState extends State<Priority_sc> {
  ...
  Future<void> fetchSigns() async {
    ...
    try {
      final response = await http.get(Uri.parse('${Config.baseUrl}/panneauxRoutes/exemple/priorite'));
      ...
      if (response.statusCode == 404) {
        ...
        // Save data to Hive
        await box.put('signs_priorite', signsList);
...
// Function to load signs from Hive if offline or fetching fails
  void loadFromHive(Box box) {
    final cachedData = box.get('signs_priorite', defaultValue: []);
