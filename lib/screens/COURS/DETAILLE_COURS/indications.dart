...
import 'INDIscreen.dart';
import '/widgets/DETAILS.dart';
import '/config.dart';
import 'package:hive/hive.dart';
class IntroductionScreenindication extends StatefulWidget {
  @override
  _IntroductionScreenDANGERState createState() => _IntroductionScreenDANGERState();
}

class _IntroductionScreenDANGERState extends State<IntroductionScreenindication> {
  String? category;
  String? explanation;
  bool isLoading = true;
  bool hasError = false;
  ...
  Future<void> fetchIntroductionData() async {
    final box = await Hive.openBox('panneauxRoutesIndicationBox');

    try {
      final response = await http.get(Uri.parse('${Config.baseUrl}/panneauxRoutes/explication/indication'));
      ...
      loadFromHive(box); // Fallback to loading from Hive if there's an error
    }
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [  SizedBox(height: 20),
              ...
              ElevatedButton(
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => indication_sc()));
                },
