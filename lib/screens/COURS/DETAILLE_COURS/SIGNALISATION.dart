...
class SignalisationScreen extends StatefulWidget {
  @override
  _SignalisationScreenState createState() => _SignalisationScreenState();
}

class _SignalisationScreenState extends State<SignalisationScreen> {
  ...
  Future<void> fetchData() async {
    final box = await Hive.openBox('feuxBox');

    try {
      final paragraphResponse =
      await http.get(Uri.parse('${Config.baseUrl}/feux/paragraphes'));
      final imageResponse =
      await http.get(Uri.parse('${Config.baseUrl}/feux/images'));
  ...
  List<Widget> buildContent() {
    ...
    for (int i = 0; i < itemCount; i++) {
      if (i < paragraphs.length) {
        contentList.add(highlightText(paragraphs[i], searchText));
  ...
  Widget highlightText(String text, String query) {
    ...
    while (start < text.length) {
      ...
      spans.add(
        TextSpan(
          text: query,
          style: TextStyle(color:Color(0xFF00D533), fontWeight: FontWeight.bold, fontSize: 18),
        ),
      );
