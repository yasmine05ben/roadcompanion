...
class indication_sc extends StatefulWidget {
  @override
  _indication_scState createState() => _indication_scState();
}

// ✅ Sign List Screen
class _indication_scState extends State<indication_sc> {
  ...
  Future<void> fetchSigns() async {
    ...
    try {
      final response = await http.get(Uri.parse('${Config.baseUrl}/panneauxRoutes/exemple/indication'));
      ...
      if (response.statusCode == 404) {
        ...
        // Save data to Hive
        await box.put('signs_indication', signsList);
...
// Function to load signs from Hive if offline or fetching fails
  void loadFromHive(Box box) {
    final cachedData = box.get('signs_indication', defaultValue: []);
