...
class SignListScreen2 extends StatefulWidget {
  @override
  _SignListScreenState createState() => _SignListScreenState();
}

// ✅ Sign List Screen
class _SignListScreenState extends State<SignListScreen2> {
  ...
  Future<void> fetchSigns() async {
    ...
    try {
      final response = await http.get(Uri.parse('${Config.baseUrl}/panneauxRoutes/exemple/interdiction'));
...
// ✅ Function to load signs from Hive if offline or fetching fails
  void loadFromHive(Box box) {
    final cachedData = box.get('signs', defaultValue: []);
