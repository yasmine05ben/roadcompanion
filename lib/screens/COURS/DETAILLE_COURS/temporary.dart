...
class IntroductionScreentmp extends StatefulWidget {
  @override
  _IntroductionScreenDANGERState createState() => _IntroductionScreenDANGERState();
}

class _IntroductionScreenDANGERState extends State<IntroductionScreentmp> {
  String? category;
  String? explanation;
  bool isLoading = true;
  bool hasError = false;
  ...
  Future<void> fetchIntroductionData() async {
    final box = await Hive.openBox('panneauxRoutesTempBox');

    try {
      final response = await http.get(Uri.parse('${Config.baseUrl}/panneauxRoutes/explication/temp'));
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [  SizedBox(height: 20),
              ...
              ElevatedButton(
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => temp_sc()));
                },
