...
import 'OBLIscreen.dart';
import '/widgets/DETAILS.dart';

import '/config.dart';
import 'package:hive/hive.dart';
class IntroductionScreenoblg extends StatefulWidget {
  @override
  _IntroductionScreenDANGERState createState() => _IntroductionScreenDANGERState();
}

class _IntroductionScreenDANGERState extends State<IntroductionScreenoblg> {
  String? category;
  String? explanation;
  bool isLoading = true;
  bool hasError = false;
  ...
  Future<void> fetchIntroductionData() async {
    final box = await Hive.openBox('panneauxRoutesObligationBox');

    try {
      final response = await http.get(Uri.parse('${Config.baseUrl}/panneauxRoutes/explication/obligation'));
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [  SizedBox(height: 20),
              ...
              ElevatedButton(
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => obligation_sc()));
                },
