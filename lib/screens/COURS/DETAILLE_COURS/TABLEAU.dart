...
class TableauScreen extends StatefulWidget {
  @override
  _TableauScreenState createState() => _TableauScreenState();
}
...
class _TableauScreenState extends State<TableauScreen> {
  ...
  Future<void> fetchData() async {
    final box = await Hive.openBox('tdbsBox');

    try {
      final paragraphResponse =
      await http.get(Uri.parse('${Config.baseUrl}/tdbs/paragraphes'));
      final imageResponse =
      await http.get(Uri.parse('${Config.baseUrl}/tdbs/images'));
  ...
  Widget highlightText(String text, String query) {
    ...
    while (start < text.length) {
      ...
      spans.add(
        TextSpan(
          text: query,
          style: TextStyle(color: Color(0xFFC52C38), fontWeight: FontWeight.bold, fontSize: 18),
        ),
      );
