...
class MarquageScreen extends StatefulWidget {
  @override
  _MarquageScreenState createState() => _MarquageScreenState();
}

class _MarquageScreenState extends State<MarquageScreen> {
  ...
  Future<void> fetchData() async {
    final box = await Hive.openBox('marquagesBox');

    try {
      final paragraphResponse =
      await http.get(Uri.parse('${Config.baseUrl}/marquages/paragraphes'));
      final imageResponse =
      await http.get(Uri.parse('${Config.baseUrl}/marquages/images'));
  ...
  Widget highlightText(String text, String query) {
    ...
    while (start < text.length) {
      ...
      spans.add(
        TextSpan(
          text: query,
          style: TextStyle(color: Color(0xFFC4931D), fontWeight: FontWeight.bold, fontSize: 18),
        ),
      );
