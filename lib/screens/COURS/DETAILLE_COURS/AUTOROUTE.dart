...
class autorouteScreen extends StatefulWidget {
  @override
  _autoScreenState createState() => _autoScreenState();
}

class _autoScreenState extends State<autorouteScreen> {
  ...
  Future<void> fetchData() async {
    final box = await Hive.openBox('autoroutesBox');

    try {
      final paragraphResponse =
      await http.get(Uri.parse('${Config.baseUrl}/autoroutes/paragraphes'));
      final imageResponse =
      await http.get(Uri.parse('${Config.baseUrl}/autoroutes/images'));
  ...
  List<Widget> buildContent() {
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      if (i < images.length) {
        contentList.add(
          Image.asset(images[i], width: 430, height: 170, fit: BoxFit.contain),
        );
        contentList.add(SizedBox(height: 20));
      }
    }
  ...
  Widget highlightText(String text, String query) {
    ...
    while (start < text.length) {
      ...
      spans.add(
        TextSpan(
          text: query,
          style: TextStyle(color: Color(0xFF00D533), fontWeight: FontWeight.bold, fontSize: 20),
        ),
      );
