...
class ArretStatScreen extends StatefulWidget {
  @override
  _ArretStatScreenState createState() => _ArretStatScreenState();
}

class _ArretStatScreenState extends State<ArretStatScreen> {
  ...
  Future<void> fetchData() async {
    final box = await Hive.openBox('arretstatsBox');

    try {
      final paragraphResponse =
      await http.get(Uri.parse('${Config.baseUrl}/arretstats/paragraphes'));
      final imageResponse =
      await http.get(Uri.parse('${Config.baseUrl}/arretstats/images'));
  ...
  Widget highlightText(String text, String query) {
    ...
    while (start < text.length) {
      ...
      spans.add(
        TextSpan(
          text: query,
          style: TextStyle(color: Color(0xFFF9844A), fontWeight: FontWeight.bold, fontSize: 20),
        ),
      );
