...
class obligation_sc extends StatefulWidget {
  @override
  _obligation_scSTate createState() => _obligation_scSTate();
}

// ✅ Sign List Screen
class _obligation_scSTate extends State<obligation_sc> {
  ...
  Future<void> fetchSigns() async {
    ...
    try {
      final response = await http.get(Uri.parse('${Config.baseUrl}/panneauxRoutes/exemple/obligation'));
      ...
      if (response.statusCode == 404) {
        ...
        // Save data to Hive
        await box.put('signs_obligation', signsList);
...
// Function to load signs from Hive if offline or fetching fails
  void loadFromHive(Box box) {
    final cachedData = box.get('signs_obligation', defaultValue: []);
